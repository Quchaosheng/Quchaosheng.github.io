...
以“相机看到 AprilTag，底盘停到指定距离”为例，真正要接通的是时间戳、模型、规划、控制和急停，而不是一个孤立的大模型。NVIDIA 的 Cosmos、Isaac Sim、Isaac ROS、TensorRT 和 Jetson 分别覆盖数据、仿真、机器人计算图、推理和边缘算力；ROS 2、控制器与安全 I/O 决定这些组件在异常时怎样停下来。
...
**证据边界：**本文是系统分层和验收方法的技术整理，没有给出某套 NVIDIA 硬件、模型或机器人平台的实际成功率。具体延迟、温度和急停行为必须通过目标设备的录包与故障注入确认。
