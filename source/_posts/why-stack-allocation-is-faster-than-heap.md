...
tags:
  ...
  - C++
  - 操作系统
description: 从分配算法、生命周期、缓存局部性和线程并发几个角度，理解栈分配与堆分配的性能差异。
---
...
`malloc`、`new` 等堆分配接口面对的请求大小和生命周期都不固定。一次分配可能涉及：
...
4. 处理内存对齐；
5. 在多线程环境中进行同步；
6. 必要时通过 `brk`、`mmap` 等机制向操作系统申请更多虚拟内存。
...
需要特别区分：调用 `new` 或 `malloc` **通常不会每次都直接触发系统调用**。大多数请求先由进程内的内存分配器从已管理的内存池中满足。
...
参考原文：[大厂二面深挖：为何栈的内存分配比堆更快？](https://mp.weixin.qq.com/s/LPcn8lnvz0E0iAwA_SOjQQ)，公众号“深度 Linux”。
